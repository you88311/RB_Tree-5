...
int real_delete_count = 0; //실제 delete횟수
int no_exist_delete_count = 0; //delete하려 했으나 노드가 존재하지 않아 delete하지 못한 횟수
...
void tree_delete(int data, RB_Tree* self, Node* tree)
{
	...
	{	
		no_exist_delete_count++; //존재하지 않은 노드 삭제하려 한 횟수 count
		real_delete_count--;      //실제로 삭제하지 않았으므로 원 상태로 바꿔줌
		return;
	}
